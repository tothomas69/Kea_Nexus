...
from helpers import get_client, load_leases, load_pool_stats, load_config, load_status, leases_to_df, fmt_ttl
...
def confirm_disable_dialog() -> None:
    st.error("Disabling DHCP immediately stops Kea from responding to all "
             "DISCOVER and REQUEST packets. Clients cannot renew or obtain leases.")
    st.warning("Existing leases are NOT revoked. Devices keep their IPs until "
               "their leases expire, but no new addresses will be issued.")
    max_p = st.number_input(
        "Auto re-enable after (seconds, 0 = manual only)",
        min_value=0, max_value=3600, value=300, step=60)
    c1, c2 = st.columns(2)
    with c1:
        if st.button("Confirm - Disable DHCP", type="primary",
...
def confirm_wipe_dialog(lease_count: int) -> None:
    st.error(
        f"WARNING: This will permanently delete all {lease_count} leases "
        "from Kea's database."
    )
    st.warning(
    ...
    c1, c2   = st.columns(2)
    with c1:
        if st.button("Wipe all leases", type="primary",
                     use_container_width=True, disabled=not can_wipe):
            try:
...
def render_maintenance(leases: list[dict]) -> None:
    ...
    dec = [l for l in leases if l.get("state", 0) == 1]
    ...
    if not dec:
        ...
        if st.button(f"Clear all {len(dec)} declined"):
            ...
            for l in dec:
                try:
                    get_client().delete_lease(l["ip-address"])
                except KeaError as e:
                    errors.append(str(e))
            load_leases.clear()
            load_pool_stats.clear()
        ...
        now = int(_time.time())
        for l in dec:
            ttl = l.get("cltt", 0) + l.get("valid-lft", 86400) - now
            r1, r2, r3 = st.columns([3, 3, 1])
            r1.code(l["ip-address"])
            r2.write(f"Quarantine: {fmt_ttl(ttl)}")
            with r3:
                if st.button("Clear", key=f"clr_{l['ip-address']}"):
                    try:
                        get_client().delete_lease(l["ip-address"])
                        load_leases.clear()
                        load_pool_stats.clear()
    ...
    with ls1:
        stype = st.selectbox("By", ["MAC Address", "Hostname"],
                             label_visibility="collapsed")
    with ls2:
        sval = st.text_input("Value", placeholder="Enter value...",
                             label_visibility="collapsed",
