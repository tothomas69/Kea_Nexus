...
from helpers import get_client, load_leases, load_pool_stats, leases_to_df, fmt_ttl
...
def _build_type_sets(config: Optional[dict]) -> tuple[set, set, set]:
	...
	reserved_macs = {r["hw-address"].lower() for r in res
	                 if "hw-address" in r and "ip-address" not in r}
	name_hosts    = {r["hostname"].lower()   for r in res
...
def add_lease_dialog() -> None:
	...
	valid_lft = st.number_input("Duration (seconds)",
	                            min_value=60, max_value=604800,
	                            value=86400, step=3600)
	c1, c2 = st.columns(2)
	with c1:
		if st.button("Add lease", type="primary", use_container_width=True):
			if not ip.strip() or not mac.strip():
				...
				return
			try:
				get_client().add_lease(
					ip.strip(), mac.strip(), hostname.strip(), int(valid_lft))
				load_leases.clear()
...
def edit_lease_dialog(lease: dict) -> None:
	...
	vlt = st.number_input("Duration (seconds)",
	                      min_value=60, max_value=604800,
	                      value=86400, step=3600)
...
def render_leases(leases: list[dict], config: Optional[dict] = None) -> None:
	...
	sorted_leases = sorted(
		leases,
		key=lambda l: KeaClient.ip_to_int(l.get("ip-address", "0.0.0.0")),
	)

	fc, cc = st.columns([5, 1])
	with fc:
		q = st.text_input("Filter", placeholder="IP, hostname or MAC...",
	...
	if q:
		q_lower = q.lower()
		sorted_leases = [
			l for l in sorted_leases
			if q_lower in l.get("ip-address", "").lower()
			or q_lower in (l.get("hostname") or "").lower()
			or q_lower in (l.get("hw-address") or "").lower()
		]
	...
	with st.expander("Advanced lease operations"):
		...
		with lc1:
			ltype = st.selectbox("Lookup by",
			                     ["IP Address", "MAC Address", "Hostname"],
			                     label_visibility="collapsed")
		with lc2:
			lval = st.text_input("Value", placeholder="Enter value...",
			                     label_visibility="collapsed",
		...
		with ac1:
			if st.button("+ Add lease manually", use_container_width=True):
				add_lease_dialog()
		with ac2:
			eip = st.text_input("Edit lease by IP", placeholder="172.16.17.x",
			                    label_visibility="collapsed")
